...
import { buildTeamStats } from "@/lib/teamStatsCompute";
...
export function TeamProfileScreen() {
  ...
  // before teams became documents) carry its NAME instead — buildTeamStats
  // accepts either. useParams() returns the segment still percent-encoded, so
  // decode it; decodeURIComponent is a safe no-op when the value is already
  // plain text.
  const teamKey = (() => {
  ...
  // A team's profile spans every season it ever fielded a lineup in, so this is
  // the league's whole history — read raw, and rolled up here rather than on a
  // serverless function that used to replay it per visitor.
  const { index, error: bundleError } = useRawBundle({ scope: "league", enabled: !!teamKey });
  const computed = useMemo(
    () => (index && teamKey ? buildTeamStats(index, { team: teamKey }) : null),
    [index, teamKey],
  );
