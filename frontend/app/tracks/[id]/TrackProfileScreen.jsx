...
import { buildTrackProfile } from "@/lib/trackCompute";
...
export function TrackProfileScreen() {
  const { id } = useParams();
  const { gameId, seriesId, seasonId, classId, className } = useLeague();
  const [tab, setTab] = useState("records"); // "records" | "results"

  // A venue's history spans every season that ever raced here, so this is the
  // league bundle — with the time-trial pools folded in, since a hot lap set in
  // a trial competes for the track record on equal terms with a race lap.
  const { index, error: bundleError } = useRawBundle({
    scope: "league", include: "time_trials", enabled: !!id,
  });

  // Everything the page shows, derived here. The scope comes straight from the
  // top-of-page Game/Series/Season/Class dropdowns: a selected class narrows the
  // leaderboard, the winners list and the headline record to that class, while
  // the per-game and per-class record breakdowns stay whole, since they exist to
  // show the categories side by side. Changing any of those re-runs this over
  // documents already in memory instead of re-reading the league.
  const track = index?.trackById[id] ?? null;
  const data = useMemo(() => {
    if (!index || !track) return null;
    return buildTrackProfile(index, {
      trackId: track.id,
      trackName: track.name,
      scope: { gameId, seriesId, seasonId, classId, className },
    });
  }, [index, track, gameId, seriesId, seasonId, classId, className]);

  const notFound = !!index && !track;
  if (bundleError || notFound) {
    return (
      <div className="empty-state">
        <span className="empty-state-icon">🏁</span>
        <p>{bundleError || "Track not found"}</p>
      </div>
    );
  }
  if (!data) return <div className="skeleton" style={{ height: 280 }} />;

  const { races_held, seasons_raced, drivers, winners, record, records_by_game = [], records_by_class = [] } = data;
