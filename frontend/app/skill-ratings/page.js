...
export default function SkillRatingsPage() {
  ...
  // the top dropdowns wins; "All …" widens the leaderboard.
  const active = seasonId
    ...
    : seriesId
      ...
      : gameId
        ...
        : { params: "scope=league", title: "League Skill Ratings" };

  useEffect(() => {
    if (loading) return;
    setData(null);
    setError(null);
    api(`/api/skill-ratings?${active.params}`).then(setData).catch(err => setError(err.message));
  ...
  return (
    <section>
      ...
      <p style={{ marginTop: 4, color: "var(--ink-1)", fontSize: "0.88rem" }}>
        Every driver&apos;s Elo-style Skill Rating, earned and lost against the Strength of Field they
        race. Everyone starts at {SR_BASELINE}. Use the Game / Series / Season menus above to change
        scope (pick &quot;All&quot; to widen it); click a column to sort.
      </p>
