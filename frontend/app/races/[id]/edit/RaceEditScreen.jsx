...
function UnifiedEditInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // The event, its season, its roster and its classes all come out of one
        // raw bundle — the same one the results page reads (see
        // components/useRawBundle.js). Nothing computed: this screen scores its
        // own preview client-side from the season's points structure.
        const res = await fetch(rawBundlePath({ scope: "race", raceId: id }));
        const bundle = await res.json();
        if (!res.ok) throw new Error(bundle?.error || "Event not found");
        if (cancelled) return;
        const ev = { event: bundle.races.find(r => r.id === id), season: bundle.seasons[0] || null };
        if (!ev.event) throw new Error("Event not found");
        setRace({ id: ev.event.id, ...ev.event });
        setSeason(ev.season || null);
        setSeasonId(ev.event.season_id);
        // Raw documents arrive in whatever order the database hands them back,
        // so the alphabetical order these pickers read in is applied here —
        // sorting is presentation, and this is where presentation happens now.
        const byName = (a, b) => String(a.name ?? "").localeCompare(String(b.name ?? ""));
        if (!cancelled) {
          setEntries([...bundle.entries].sort(byName));
          setClasses(bundle.classes);          // already in Class-menu order
          setDrivers([...bundle.drivers].sort(byName));
          setTemplates([...normalizedBuiltinTemplates(), ...[...bundle.points_templates].sort(byName)]);
          setSeries(bundle.series[0] || null);
          setSeriesName(bundle.series[0]?.name ?? "");
        }
      } catch (err) {
...
export function RaceEditScreen() {
  return (
    <AdminGate>
