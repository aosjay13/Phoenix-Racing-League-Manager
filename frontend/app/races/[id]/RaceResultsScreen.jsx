...
import { buildEvent } from "@/lib/eventCompute";
...
export function RaceResultsScreen() {
  ...
  // The season around this event, raw. Its results grid, its points and its
  // roster are all derived below — the numbers on this page have to be the ones
  // the standings total, and they are because both run the same scorer.
  const { index, error: bundleError, reload: load } = useRawBundle({
    scope: "race", raceId: id, enabled: !!id,
  });
  const computed = useMemo(
    () => (index && id ? buildEvent(index, { raceId: id }) : null),
    [index, id],
  );
  const data = computed?.status === 200 ? computed.body : null;
  const loadError = error || bundleError
    || (computed && computed.status !== 200 ? computed.body?.error : null);
  ...
  useEffect(() => {
    if (!data) return;
    const withResults = data.races.find(s => s.results.length);
    const wanted = readParam("session");
    const linked = wanted === "Qualifying" && data.qualifying.length
      ? "__qual"
      : data.races.find(s => s.name === wanted)?.name ?? null;
    setTab(prev => prev ?? linked ?? (withResults ? withResults.name : (data.qualifying.length ? "__qual" : data.races[0]?.name ?? null)));
  }, [data]);
  ...
  if (loadError) return <div className="empty-state"><span className="empty-state-icon">🏁</span><p>{loadError}</p></div>;
