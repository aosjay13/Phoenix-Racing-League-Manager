...
// Both wrap their feed in the same { status, body } envelope the other
// builders use, so one storage and rebuild path can handle all of them.
export async function computeSeasonSchedule(leagueId, { seasonId, classId, className }) {
  return { status: 200, body: await oneSeason(seasonId, classId, className) };
}

export async function computeGlobalSchedule(leagueId, { gameId, seriesId }) {
  return { status: 200, body: await globalFeed(gameId || null, seriesId || null, leagueId) };
}
